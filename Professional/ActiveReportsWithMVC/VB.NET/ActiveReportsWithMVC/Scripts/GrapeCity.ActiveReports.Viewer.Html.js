!function(window) {
    ...
    function ArsRestReportService(service, serviceUrl, options, resourceManager) {
        ...
        function getPageCount(reqId) {
            function getPageCountImpl(isFirstRequest) {
                param = "", isFirstRequest && (param = "&timeout=30000&throttle=80"), $base.get("reports/jobs/render/" + reqId + "/pages/available" + getRandomParameter() + param).done(function(result, status, xhr) {
                    202 === xhr.status ? getPageCountImpl(!1) : result.Cancelled ? result.Complete ? def.resolve(result.AvailablePages, DocumentState.cancelled) : (def.notify(result.AvailablePages, DocumentState.cancelling), 
                    getPageCountImpl(!1)) : result.Complete ? def.resolve(result.AvailablePages, DocumentState.completed) : (def.notify(result.AvailablePages), 
                    getPageCountImpl(!1));
                }).fail(function(problem) {
                    def.reject(problem);
                });
            ...
            var def = $.Deferred();
            return getPageCountImpl(!0), def.promise();
        }
        function getDelayedContent(pageNumber, reqId) {
    ...
    function BrowserSpecific() {
        ...
        function chromeSupportVersion() {
            var v = navigator.userAgent.match(/chrome\/(\d+)(?:\.\d+)+/i);
            return !v || parseInt(v[1]) < 77;
        }
        return {
            SupportPdfFrame: (isChrome() || isSafari()) && chromeSupportVersion() && !isEdge()
        };
    }
    ...
    function ReportModel(reportService, reportInfo, busy, resourceManager, renderMode, previewPages) {
        ...
        function run(ignorePreviewPages) {
            ...
            if (!allParametersOk) throw new Error("Invalid parameters!");
        ...
        return {
            ...
            email: {
                ...
                visible: ko.computed(function() {
                    return viewer.document().canSendEmail && viewer.document().canSendEmail();
                })
            },
            toc: {
                exec: function() {
